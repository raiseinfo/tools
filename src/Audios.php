...
class Audios
{
    ...
    //
    // public function hasAudio($url, $silenceThreshold = -50, $silenceDuration = 0.1, $silenceRatioThreshold = 0.8)
    // {
    //     $filePath = tempnam(sys_get_temp_dir(), 'has_audio_' . time());
    //     $content = file_get_contents($url);
    //     if ($content === false) {
    //         throw new \think\exception\ValidateException('读取文件失败');
    //     }
    ...
    //     );
    //
    //     \think\facade\Log::record('$command');
    //     \think\facade\Log::record($command);
    //
    //     // 执行命令并捕获输出
    //     exec($command, $output, $returnVar);
    //
    //     \think\facade\Log::record('$returnVar');
    //     \think\facade\Log::record($returnVar);
    //     \think\facade\Log::record('$output');
    //     \think\facade\Log::record($output);
    //
    //     // 检查命令是否成功
    //     if ($returnVar !== 0) {
    //         throw new \Exception("FFmpeg command failed with return code: " . $returnVar);
    //     }
    //
    //     // 获取文件的总时长
    ...
    //
    //     if ($durationReturnVar !== 0) {
    //         throw new \Exception("FFprobe command failed with return code: " . $durationReturnVar);
    //     }
    //
    //     $totalDuration = floatval(trim(implode("\n", $durationOutput)));
    //     \think\facade\Log::record('Total Duration: ' . $totalDuration . ' seconds');
    //
    //     // 如果文件总时长为 0，直接返回有声音（避免除以 0）
    //     if ($totalDuration <= 0) {
    //         return true;
    //     }
    //
    ...
    //     // 计算静音比例
    //     $silenceRatio = $totalSilenceDuration / $totalDuration;
    //     \think\facade\Log::record('Silence Ratio: ' . ($silenceRatio * 100) . '%');
    //
    //     // 判断是否有声音
    //     $hasSound = $silenceRatio < $silenceRatioThreshold;
    //
    //     \think\facade\Log::record('$hasSound');
    //     if ($hasSound) {
    //         \think\facade\Log::record('有声');
    //     } else {
    //         \think\facade\Log::record('无声');
    //     }
    //
    //     // 删除本地文件
    ...
    //
    //     // 返回音量信息
    //     return $hasSound; // 假设 -20 表示有声音，-90 表示静音
    // }
    //
    //
    // /**
